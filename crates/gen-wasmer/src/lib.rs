...
impl Wasmer {
    ...
    fn print_intrinsics(&mut self) {
        if self.needs_raw_mem {
            self.push_str("use witx_bindgen_wasmer::rt::RawMem;\n");
        }
        if self.needs_char_from_i32 {
            self.push_str("use witx_bindgen_wasmer::rt::char_from_i32;\n");
        }
        if self.needs_invalid_variant {
            self.push_str("use witx_bindgen_wasmer::rt::invalid_variant;\n");
        }
        if self.needs_bad_int {
            self.push_str("use core::convert::TryFrom;\n");
            self.push_str("use witx_bindgen_wasmer::rt::bad_int;\n");
        }
        if self.needs_validate_flags {
            self.push_str("use witx_bindgen_wasmer::rt::validate_flags;\n");
        }
        if self.needs_le {
            self.push_str("use witx_bindgen_wasmer::Le;\n");
        }
        if self.needs_copy_slice {
            self.push_str("use witx_bindgen_wasmer::rt::copy_slice;\n");
        }
    }
    ...
    fn print_lib_buffer(
        ...
        if self.in_import {
            ...
            self.push_str(&format!(
                "witx_bindgen_wasmer::imports::{}Buffer<{}, ",
...
impl Generator for Wasmer {
    fn preprocess_one(&mut self, iface: &Interface, dir: Direction) {
        ...
        self.src
            .push_str("#[allow(unused_imports)]\nuse witx_bindgen_wasmer::{anyhow, wasmer};\n");
    ...
    fn type_record(
        ...
        if record.is_flags() {
            self.src
                .push_str("witx_bindgen_wasmer::bitflags::bitflags! {\n");
            self.rustdoc(docs);
            self.src
        ...
        if self.modes_of(iface, id).len() > 0
            ...
            && !record.is_tuple()
        {
            self.src.push_str("impl witx_bindgen_wasmer::Endian for ");
            self.src.push_str(&name.to_camel_case());
            self.src.push_str(" {\n");
            ...
            // predicate).
            self.src
                .push_str("unsafe impl witx_bindgen_wasmer::AllBytesValid for ");
            self.src.push_str(&name.to_camel_case());
            self.src.push_str(" {}\n");
        }
    }
    ...
    fn type_resource(&mut self, iface: &Interface, ty: ResourceId) {
        ...
        self.src.push_str(&format!(
            "pub struct {}(witx_bindgen_wasmer::rt::ResourceIndex);\n",
            tyname
        ));
    }
    ...
    fn import(&mut self, iface: &Interface, func: &Function) {
        ...
        let mut self_arg = "&self".to_string();
        if func_takes_all_memory {
            self_arg.push_str(", mem: witx_bindgen_wasmer::RawMemory");
        }
        self.in_trait = true;
        ...
        if self.opts.tracing {
            self.src.push_str(&format!(
                "
                    let span = witx_bindgen_wasmer::tracing::span!(
                        witx_bindgen_wasmer::tracing::Level::TRACE,
                        \"witx-bindgen abi\",
                        module = \"{}\",
                        function = \"{}\",
                    );
        ...
        for name in needs_functions.keys() {
            self.src
                .push_str(&format!("let func_{0} = env.func_{0}.get_ref().unwrap();\n", name));
        }
        ...
        if needs_borrow_checker {
            ...
            // of WasmPtr/WasmCell.
            self.src.push_str(
                "let mut _bc = unsafe { witx_bindgen_wasmer::BorrowChecker::new(env.memory.get_ref().unwrap().data_unchecked_mut()) };\n",
            );
        }
    ...
    fn export(&mut self, iface: &Interface, func: &Function) {
        ...
        for (name, func) in needs_functions {
            ...
            exports.fields.insert(format!("func_{}", name), (func.ty(), get));
        }
    ...
    fn finish_one(&mut self, iface: &Interface, files: &mut Files) {
        for (module, funcs) in sorted_iter(&self.imports) {
            ...
            let is_async = !self.opts.async_.is_none();
            if is_async {
                self.src.push_str("#[witx_bindgen_wasmer::async_trait]\n");
            }
            ...
            if self.opts.custom_error {
                ...
                if self.needs_custom_error_to_trap {
                    self.src.push_str(
                        "fn error_to_trap(&self, err: Self::Error) -> wasmer::RuntimeError;\n",
                    );
                }
                for ty in self.needs_custom_error_to_types.iter() {
                    self.src.push_str(&format!(
                        "fn error_to_{}(&self, err: Self::Error) -> Result<{}, wasmer::RuntimeError>;\n",
                        ty.to_snake_case(),
                        ty.to_camel_case(),
                    ));
                }
            ...
            if self.all_needed_handles.len() > 0 {
                ...
                for handle in self.all_needed_handles.iter() {
                    ...
                    self.src.push_str("_table: witx_bindgen_wasmer::Table<T::");
                    self.src.push_str(&handle.to_camel_case());
                    self.src.push_str(">,\n");
                }
                self.src.push_str("}\n");
        ...
        for (module, funcs) in mem::take(&mut self.imports) {
            ...
            self.push_str("_to_imports<T>(store: &wasmer::Store, imports: &mut wasmer::ImportObject, data: T)\n");
        ...
        for (module, exports) in sorted_iter(&mem::take(&mut self.exports)) {
            ...
            for r in self.exported_resources.iter() {
                self.src.push_str(&format!(
                    "
                        index_slab{idx}: witx_bindgen_wasmer::rt::IndexSlab,
                        resource_slab{idx}: witx_bindgen_wasmer::rt::ResourceSlab,
            ...
            //     self.push_str("buffer_glue: witx_bindgen_wasmer::exports::BufferGlue,");
            // }
            self.push_str("}\n");
            ...
            //     self.push_str(
            //         "
            //             use witx_bindgen_wasmer::rt::get_memory;

            //             let buffer_glue = witx_bindgen_wasmer::exports::BufferGlue::default();
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::I64FromU64 | Instruction::I64FromS64 => {
                ...
                results.push(format!("witx_bindgen_wasmer::rt::as_i64({})", s));
            }
            ...
            | Instruction::I32FromS32 => {
                let s = operands.pop().unwrap();
                results.push(format!("witx_bindgen_wasmer::rt::as_i32({})", s));
            }
            ...
            Instruction::BufferLiftPtrLen { push, ty } => {
                ...
                if iface.all_bits_valid(ty) {
                    ...
                    if *push {
                        ...
                        results.push(format!(
                            "witx_bindgen_wasmer::imports::PushBuffer::new(
                                &mut _bc, ptr{}, len{}, {}, &{})?",
                        ...
                        results.push(format!(
                            "witx_bindgen_wasmer::imports::PullBuffer::new(
                                &mut _bc, ptr{}, len{}, {}, &{})?",
                            tmp, tmp, size, closure
                        ));
                    }
            ...
            Instruction::CallInterface { module: _, func } => {
                ...
                if self.gen.opts.tracing && func.params.len() > 0 {
                    self.push_str("witx_bindgen_wasmer::tracing::event!(\n");
                    self.push_str("witx_bindgen_wasmer::tracing::Level::TRACE,\n");
                    for (i, (name, _ty)) in func.params.iter().enumerate() {
                        self.push_str(&format!(
                            "{} = witx_bindgen_wasmer::tracing::field::debug(&param{}),\n",
                            to_rust_ident(name),
                            i
                        ));
                    }
                ...
                if self.func_takes_all_memory {
                    let mem = self.memory_src();
                    self.push_str("let raw_memory = witx_bindgen_wasmer::RawMemory { slice: ");
                    self.push_str(&mem);
                    self.push_str(".raw() };\n");
                }
                ...
                if self.gen.opts.tracing && func.results.len() > 0 {
                    ...
                    for name in results.iter() {
                        self.push_str(&format!(
                            "{} = witx_bindgen_wasmer::tracing::field::debug(&{0}),\n",
                            name,
                        ));
                    }
                    self.push_str(");\n");
