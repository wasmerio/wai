...
fn execute(name: &str, wasm: &Path, py: &Path, imports: &Path, exports: &Path) {
    ...
    let iface = wasmer_wit_bindgen_gen_core::wit_parser::Interface::parse_file(imports).unwrap();
    ...
    let iface = wasmer_wit_bindgen_gen_core::wit_parser::Interface::parse_file(exports).unwrap();
    let mut files = Default::default();
    wit_bindgen_gen_wasmer_py::Opts::default()
        .build()
        .generate_all(&[iface], &[], &mut files);
