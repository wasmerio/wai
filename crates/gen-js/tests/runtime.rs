...
fn execute(name: &str, wasm: &Path, ts: &Path, imports: &Path, exports: &Path) {
    ...
    let imports = wasmer_wit_bindgen_gen_core::wit_parser::Interface::parse_file(imports).unwrap();
    let exports = wasmer_wit_bindgen_gen_core::wit_parser::Interface::parse_file(exports).unwrap();
